<?php
$_lang['dbadmin'] = 'dbAdmin';
$_lang['dbadmin_menu_desc'] = 'Управление таблицами БД';
$_lang['dbadmin_intro_msg'] = 'Вы можете выгрузить всю базу данных целиком или только выделенные таблицы. Также таблицы можно очистить или удалить.';
$_lang['dbadmin_grid_search'] = 'Поиск';

$_lang['dbadmin_tables'] = 'Таблицы';
$_lang['dbadmin_sql'] = 'SQL';
$_lang['dbadmin_table'] = 'Таблица';
$_lang['dbadmin_table_type'] = 'Тип';
$_lang['dbadmin_table_collation'] = 'Сравнение';
$_lang['dbadmin_table_rows'] = 'Строки';
$_lang['dbadmin_table_size'] = 'Размер, Кb';
$_lang['dbadmin_table_actions'] = 'Действия';
$_lang['dbadmin_enter_sql_query'] = 'Введите SQL запрос';
$_lang['dbadmin_query_result'] = 'Результат';
$_lang['dbadmin_class'] = 'Класс';
$_lang['dbadmin_package'] = 'Пакет';

$_lang['dbadmin_db_export'] = 'Экспорт базы данных';
$_lang['dbadmin_table_export'] = 'Экспорт таблицы';
$_lang['dbadmin_tables_export'] = 'Экспорт таблиц';
$_lang['dbadmin_selected_export'] = 'Экспортировать выбранное';
$_lang['dbadmin_table_remove'] = 'Удалить таблицу';
$_lang['dbadmin_tables_remove'] = 'Удалить таблицы';
$_lang['dbadmin_selected_remove'] = 'Удалить выбранное';
$_lang['dbadmin_table_remove_confirm'] = 'Вы уверены, что хотите удалить эту таблицу?';
$_lang['dbadmin_tables_remove_confirm'] = 'Вы уверены, что хотите удалить выбранные таблицы?';
$_lang['dbadmin_table_truncate'] = 'Очистить таблицу';
$_lang['dbadmin_tables_truncate'] = 'Очистить таблицы';
$_lang['dbadmin_tables_truncate_confirm'] = 'Вы уверены, что хотите удалить все записи из выбранных таблиц?';
$_lang['dbadmin_selected_truncate'] = 'Очистить выбранное';
$_lang['dbadmin_table_data'] = 'Посмотреть таблицу';
$_lang['dbadmin_rows_number'] = 'Количество записей: ';
$_lang['dbadmin_package_update'] = ' <i class="icon icon-exclamation-triangle package-update" title="Доступна новая версия dbAdmin"></i>';

$_lang['dbadmin_execute_sql'] = 'Выполнить SQL запрос';
$_lang['dbadmin_execute'] = 'Выполнить';
$_lang['dbadmin_close'] = 'Закрыть';
$_lang['dbadmin_clear'] = 'Очистить';

$_lang['createdon'] = 'Время создания: ';
$_lang['table_structure'] = 'Структура таблицы';
$_lang['table_dump'] = 'Дамп данных таблицы';

$_lang['dbadmin_table_err_ae'] = 'Таблица с таким именем уже существует!';
$_lang['dbadmin_table_err_nf'] = 'Таблица не найдена!';
$_lang['dbadmin_table_err_ns'] = 'Таблица не указана!';
$_lang['dbadmin_table_err_nfs'] = 'Не найдена таблица с id = "[[+id]]"!';
$_lang['dbadmin_table_err_path'] = 'Путь не найден!';
$_lang['dbadmin_table_err_remove'] = 'Ошибка при удалении таблицы!';
$_lang['dbadmin_table_err_save'] = 'Ошибка при сохранении таблицы!';
$_lang['dbadmin_sql_executed_success'] = 'Запрос выполнен успешно.';
$_lang['dbadmin_sql_executed_failed'] = 'Ошибка выполнения запроса!';
$_lang['dbadmin_table_err_rename'] = 'Ошибка переименования таблицы!';
$_lang['dbadmin_sync'] = 'Ошибка синхронизации таблиц!';
// log
$_lang['dbadmin_err_load_class'] = '[dbAdmin] Не получилось загрузить указанный класс!';
$_lang['dbadmin_err_path'] = '[dbAdmin] Путь для указанного пакета не найден!';
$_lang['dbadmin_table_err_class_specified'] = '[dbAdmin] Не определен класс для таблицы [[+table]]!';